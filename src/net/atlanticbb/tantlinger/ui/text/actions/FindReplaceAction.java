...
public class FindReplaceAction extends BasicEditAction {
    ...
    public FindReplaceAction(boolean isReplace, boolean _showHTML) {
        ...
        if (isReplace) {
            ...
            putValue(MNEMONIC_KEY, Integer.valueOf(i18n.mnem("replace_")));
        } else {
            putValue(NAME, i18n.str("find_")); //$NON-NLS-1$
            putValue(MNEMONIC_KEY, Integer.valueOf(i18n.mnem("find_")));
    ...
    protected void doEdit(ActionEvent e, JEditorPane textComponent) {
        ...
        if (dialog == null) {
            if (c instanceof Frame frame) {
                if (isReplaceTab) {
                    dialog = new TextFinderDialog(frame, textComponent, TextFinderDialog.REPLACE);
                } else {
                    dialog = new TextFinderDialog(frame, textComponent, TextFinderDialog.FIND);
                }
            } else if (c instanceof Dialog dialog1) {
                if (isReplaceTab) {
                    dialog = new TextFinderDialog(dialog1, textComponent, TextFinderDialog.REPLACE);
                } else {
                    dialog = new TextFinderDialog(dialog1, textComponent, TextFinderDialog.FIND);
                }
            } else {
                return;
            }
